 ...
 */
@Component
public class CatConsumerProducerModule extends AbstractPluginModuleDefinitionProcessor {
    ...
    public String pluginName() {
        return "cat-dubbo-consumer-plugin";
    }

    @Override
    public List<EnhanceClassInfo> getEnhanceClassInfos() {
        EnhanceClassInfo log = EnhanceClassInfo.builder().classPattern("com.alibaba.dubbo.rpc.protocol.AbstractInvoker").methodPatterns(EnhanceClassInfo.MethodPattern.transform("doInvoke")).includeSubClasses(true).build();
        return Lists.newArrayList(log);
    }

    @Override
    public Set<AdviceNameDefinition> getMethodAdviceInvoke() {
        return Sets.newHashSet(AdviceNameDefinition.DUBBO_CONSUMER);
    }
