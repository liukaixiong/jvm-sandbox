...
public class JarManager {
    ...
    private Map<String, Long> jarFileSignCache = new HashMap<>();
    ...
    private boolean checkLoaded(File file) {
        ...
        if (jarVersion == null || jarVersion == 0) {
            jarFileSignCache.put(key, jarVersion);
            return false;
        } else {
            long currentJarVersion = checksumCRC32(file);
            if (jarVersion == currentJarVersion) {
                return true;
            }
        }
        return false;
    ...
    class JarInfo {
        ...
        private ManagerClassLoader managerClassLoader;

    }
//    public static void main(String[] args) {
//        String jarFile = "E:\\study\\sandbox\\sandbox-module\\";
//
//        PluginInstanceFactory factory = new PluginInstanceFactory(jarFile);
//        factory.loadComponents();
//
//        List<PluginModule> list = GroupContainerHelper.getInstance().getList(PluginModule.class);
